...
###############################################################################################################################################################
  host_main_point(){
    printf "/${exp_choosen_category}/ >>> "; read opt1
            cd Host &> /dev/null # get in Host Folder

        if [[ "$opt1" == "help" ]]; then
            gnome-terminal &> /dev/null  -- sh -c "cat help_host.txt; exec bash" &
            host_main_point
            ...
            curl -s "https://api.binaryedge.io/v2/query/ip/$host_ip" -H "X-Key:$binaryedge_api_key" | jq > "$file_name"
        ...
        ip_beg_point
            ...
            result=$(curl -s "https://api.binaryedge.io/v2/query/ip/$var_hname2ip" -H "X-Key:$binaryedge_api_key") # | jq >> saved_file.txt
            now=$(date +"%Y_%m_%d_%H_%M_%S_%p")
        ...
        hname2ip_beg_point
          ...
          if [[ "$sub_name_updt" == "Free" ]]; then # if subscription is free,then do not allow history options
                echo -e "${RED}You have to have paid subscription to use history feature!${NC}"
                host_main_point
          else
                
            host_history_beg_point(){
            ...
            if [[ "$history_opt" == "ip" ]]; then
                ...
                if [[ "$host_ip" == "" ]]; then
                    ...
                    file_name=$(echo "${file_name//\//_slash_}") # in case filename has /,for error purposes change it with _slash_
                    curl -s "https://api.binaryedge.io/v2/query/ip/historical/$host_ip" -H "X-Key:$binaryedge_api_key" | jq > "$file_name"
                ...
                if read -r && read -r ; then # if the temp file has more than one lines. (more than 1 IPs)
                 ...
                 if [[ "$more_than_1ip" = 1 ]]; then # if var_hname2ip has more than one IP,then choose what IP you want to scan
                   ...
                   result=$(curl -s "https://api.binaryedge.io/v2/query/ip/historical/$var_hname2ip" -H "X-Key:$binaryedge_api_key" | jq) # | jq >> saved_file.txt
                   now=$(date +"%Y_%m_%d_%H_%M_%S_%p")
            ...
            host_search_beg_point(){            
            printf "/${exp_choosen_category}/$opt1/{query=,only_ips=} >>> "; read search_par # read search parameter ( query,page,only_ips )
             
                if [[ "$search_par" == "help_search" ]]; then
                    gnome-terminal &> /dev/null  -- sh -c "curl -s https://raw.githubusercontent.com/binaryedge/docs.binaryedge.io/master/docs/search.md; exec bash" & # print info for search parameters
                    host_search_beg_point

                elif [[ "$search_par" =~ "query=" ]] || [[ "$search_par" =~ "only_ips=" ]]; then
                    upd_search_par=$(echo "${search_par//\//%20}") # changes / to %20 for AND OR
                    result=$(curl -s "https://api.binaryedge.io/v2/query/search?${upd_search_par}" -H "X-Key:$binaryedge_api_key" | jq) 
                    now=$(date +"%Y_%m_%d_%H_%M_%S_%p")
                ...
                elif [[ "$search_par" == "stats" ]]; then # get in search/stats
                    host_search_stats_beg_point(){ 
                    printf "/${exp_choosen_category}/$opt1/search/stats{query=,type=,order=} >>> "; read search_stats_par # read search/stats parameter ( query,type,order )
                    if [[ "$search_stats_par" =~ "query=" ]] || [[ "$search_stats_par" =~ "type=" ]] || [[ "$search_stats_par" =~ "order=" ]]; then
                        upd_search_stats_par=$(echo "${search_stats_par//\//%20}") # changes / to %20 for AND OR
                        result=$(curl -s "https://api.binaryedge.io/v2/query/search/stats?${upd_search_stats_par}" -H "X-Key:$binaryedge_api_key" | jq)
                        now=$(date +"%Y_%m_%d_%H_%M_%S_%p")
